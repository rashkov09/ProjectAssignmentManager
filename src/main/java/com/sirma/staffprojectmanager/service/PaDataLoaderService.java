...
public class PaDataLoaderService implements ApplicationRunner {
	...
	public String getOverlappingProjects() {
		...
		OverlapProjectsDto
			overlapProjectsDto =
			overlapProjectsDtoList.stream().max(Comparator.comparing(OverlapProjectsDto::getOverlapDays)).orElseThrow();
		builder.append(overlapProjectsDto);
		builder.append(System.lineSeparator());
		overlapProjectsDtoList.stream().filter(
			result -> result.getEmp1().equals(overlapProjectsDto.getEmp1()) && result.getEmp2().equals(
				overlapProjectsDto.getEmp2())).forEach(project -> {
			builder.append("ProjectID: ").append(project.getProjectId()).append(", ").append("Total days: ")
			       .append(project.getTotalDays()).append("\n");
		});
